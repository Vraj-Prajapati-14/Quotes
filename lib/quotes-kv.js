...
async function initKV() {
  ...
  try {
    if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
      // Try using @upstash/redis for REST API support (recommended for Vercel KV)
      try {
        ...
        useKV = true
        console.log('✅ Using Vercel KV (Upstash Redis) for storage')
        return true
      } catch (upstashError) {
...
async function getData() {
  ...
  if (useKV && kvClient) {
    try {
      // Handle both @upstash/redis and @vercel/kv clients
      let data
      if (typeof kvClient.get === 'function') {
        data = await kvClient.get(QUOTES_KEY)
      } else {
        // Fallback for @vercel/kv
        data = await kvClient.get(QUOTES_KEY)
      }
...
async function saveData(data) {
  ...
  if (process.env.VERCEL || process.env.VERCEL_ENV) {
    const errorMsg = 'Vercel KV not configured! Please set up Vercel KV in your dashboard. See VERCEL_KV_SETUP.md for instructions. ' +
